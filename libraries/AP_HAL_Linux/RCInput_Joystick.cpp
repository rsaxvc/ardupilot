...
#define debug(fmt, args ...) \
    do { \
        fprintf(stderr,"[RCInput_Joystick]: %s:%d: " fmt "\n", __FUNCTION__, __LINE__, ## args);\
       } while (0)
#define debug_once(fmt, args ...) \
    if(!errmute) do { \
        fprintf(stderr,"[RCInput_Joystick]: %s:%d: " fmt "\n", __FUNCTION__, __LINE__, ## args);\
        errmute = true;\
        } while (0)
...
#define debug_once(fmt, args ...) do{} while(0)
#endif
...
bool RCInput_Joystick::opendev()
{
    ...
    if (!dev_input_js) {
        debug_once("No input device specified");
        return false;
    }
    ...
    if (fd < 0) {
        debug_once("Could not open %s: %s", dev_input_js, strerror(errno));
        return false;
    }

    if (ioctl(fd, JSIOCGAXES, &num_axes) < 0 ) {
        debug_once("JSIOCGAXES failed: %s", strerror(errno));
        goto failure_close_fd;
    }

    if (ioctl(fd, JSIOCGBUTTONS, &num_buttons) < 0 ) {
        debug_once("JSIOCGBUTTONS failed: %s", strerror(errno));
        goto failure_close_fd;
    }
